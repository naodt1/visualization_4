...
1. **The Wordcloud**  
For the selected nodes, we can see the wordcloud for the words that commonly occur in the overview for the movies in that cluster. The font size and the color encode how common or unique the word is. We used a diverging color scale that reads from green (very common) to orange (highly unique).
2. **The Genre chart**  
